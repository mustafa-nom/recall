...
def store_suggestion(
    ...
    # Check for near-exact duplicates (similarity > 0.88) on the same domain
    try:
        ...
        if existing["distances"] and existing["distances"][0]:
            for i, dist in enumerate(existing["distances"][0]):
                if dist > 0.12:  # cosine distance > 0.12 = similarity < 0.88
                    continue

                existing_meta = existing["metadatas"][0][i] if existing["metadatas"][0] else {}

                # Always require same domain to merge — prevents cross-site collapse
                existing_domain = existing_meta.get("site_domain", "")
                if site_domain and existing_domain and site_domain != existing_domain:
                    continue
                ...
                if site_domain and existing_domain in ("", "unknown"):
